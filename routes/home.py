...
def home():
    if request.method == "POST":
        ...
        if len(letters) > 0:
            grouped_buildable_words, pangrams, total_words = (
                word_manager.find_words_with_letters(
                    word_list, letters, word_manager.special
                )
            )

            matrix, word_grid, letterset, lengthset = word_manager.get_hint_information(
                grouped_buildable_words
            )
            ...
            return render_template(
                "index.html",
                grouped_buildable_words=grouped_buildable_words,
                pangrams=pangrams,
