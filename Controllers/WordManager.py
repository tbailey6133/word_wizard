...
class WordManager:
  ...
  def get_letters(self, letter_form):
    if letter_form:
      try:
          ...
          letters += special + outer_1 + outer_2 + outer_3 + outer_4 + outer_5 + outer_6
  ...
  def find_words_with_letters(self, word_list, letters, special):
    ...
    total_words = len({word for word in word_list if all(letter in letters for letter in word) and special in word})
  ...
  def build_word_grid_hints(self, grouped_words):
    ...
    for item in grouped_words:
        ...
        for word in words_arr:
            ...
            word_grid[item][starting_letter] = word_grid[item].get(starting_letter, 0) + 1
    return word_grid
